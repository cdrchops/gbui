...
public class BWindow extends BContainer
    ...
    /**
     * Sizes this window to its preferred size, accounting for the specified width or height hints. Specify -1 for a
     * dimension to indicate that it is freely resizable and a non-negative hint for a dimension that should be no
     * larger than a particular size. This method does not change the window's coordinates.
     */
    ...
    /**
     * Positions this window in the center of the display. This should be called after configuring the size of the
     * window (using, for example, a call to {@link #pack}).
     */
    public void center() {
    ...
    /**
     * Configures this window to be modal which causes it to "steal" all mouse and keyboard input while it is added to
     * the interface hierarchy. Mouse movement and button press events that would normally go to other windows or to the
     * default mouse target will instead be sent to the top-most modal window. Other events destined for other windows
     * (key events and mouse entry and exit events) will not be dispatched.
     */
    ...
    /**
     * Returns whether or not this window is an overlay, in which case its children will not receive input focus by
     * default.
     */
    ...
    /**
     * Configures the "layer" occupied by this window. Windows normally stack one atop another with the most recently
     * added window being highest in the stack. The layer configuration allows a window to be added above windows in a
     * lower layer regardless of when it or other windows are added. All windows default to a layer of zero, windows
     * with a higher layer will be "above" those with a lower layer. Windows in the same layer stack according to the
    ...
    /**
     * Configures this window with its root node. Do not call this method, it is called automatically when a window is
     * added to the root node via a call to {@link BRootNode#addWindow}.
     */
    ...
    private boolean beenDrug;

    private boolean draggable = true;

    private boolean isMaximized = true;
    private String maximizedStyleClass = "dradiswindow";
    private String minimizedStyleClass = "collapsedwindow";

    public boolean hasBeenDrug() {
        return beenDrug;
    }

    public void setBeenDrug(final boolean _beenDrug) {
        beenDrug = _beenDrug;
    }
    ...
    public void setMaximized(final boolean _maximized) {
